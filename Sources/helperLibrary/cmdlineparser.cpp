...
CmdLineParser::CmdLineParser(const CmdLineParser &_other)
{
    ...
    m_sourceCommandLine = _other.m_sourceCommandLine;
}
...
int CmdLineParser::parse(int argc, char **argv)
{
    ...
    for(int i = 0 ; i < argc; i++)
    {
        ...
        m_sourceCommandLine += QString(argv[i]) + " ";
    }
    m_sourceCommandLine = m_sourceCommandLine.trimmed();
...
const QString &CmdLineParser::getSourceCommandLine() const
{
    return m_sourceCommandLine;
}
